...
def task_page():
    ...
    matched = filter_history(history, query, kind)
    page = paginate(matched, request.args.get('page', 1),
                    request.args.get('per_page', DEFAULT_PAGE_SIZE))
    return render_template(
        'tasks.html', config=config, tasks=tasks,
        history=page['items'], page=page,
        page_numbers=page_numbers(page['page'], page['pages']),
        query=query, kind=kind, total_history=len(history))
