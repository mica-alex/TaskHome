...
def scf(clean_state, monkeypatch):
    ...
    class Api:
        ...
        def set_issues(self, issues, per_page=None):
            per_page = per_page or taskhome.SCF_PER_PAGE
    ...
    api = Api()
    api.set_issues([])
    monkeypatch.setattr(taskhome.requests, 'get', api.get)
    monkeypatch.setattr(taskhome, 'print_scf_issue',
...
def test_disabled_listener_does_not_poll(scf, clean_state):
    taskhome.listeners['scf']['enabled'] = False
    assert taskhome.poll_scf_listener(utc('2026-03-05T12:00:00')) == 0
    assert scf.requests == []


def test_empty_request_types_does_not_poll(scf):
    taskhome.listeners['scf']['request_types'] = '  '
    assert taskhome.poll_scf_listener(utc('2026-03-05T12:00:00')) == 0
    assert scf.requests == []


def test_missing_listener_is_harmless(clean_state):
    taskhome.listeners.clear()
    assert taskhome.poll_scf_listener(utc('2026-03-05T12:00:00')) == 0


def test_interval_is_respected(scf):
    taskhome.listeners['scf']['last_check'] = '2026-03-05T11:58:00Z'
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))  # only 2m of a 5m interval
    assert scf.requests == []

    taskhome.poll_scf_listener(utc('2026-03-05T12:04:00'))  # 6m elapsed
    assert len(scf.requests) == 1


def test_unparseable_last_check_polls_rather_than_stalling(scf):
    taskhome.listeners['scf']['last_check'] = 'garbage'
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    assert len(scf.requests) == 1
...
def test_new_issues_print(scf, clean_state):
    ...
    assert taskhome.poll_scf_listener(utc('2026-03-05T12:00:00')) == 2
...
def test_same_issue_is_not_reprinted_next_cycle(scf, clean_state):
    ...
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    taskhome.poll_scf_listener(utc('2026-03-05T12:10:00'))
...
def test_issues_without_ids_are_skipped(scf, clean_state):
    ...
    assert taskhome.poll_scf_listener(utc('2026-03-05T12:00:00')) == 1


def test_seen_list_is_bounded(scf, clean_state, monkeypatch):
    monkeypatch.setattr(taskhome, 'SCF_SEEN_LIMIT', 5)
    scf.set_issues([issue(i) for i in range(20)])
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    assert len(taskhome.listeners['scf']['seen']) == 5
...
def test_page_guard_stops_runaway_pagination(scf, clean_state, monkeypatch):
    monkeypatch.setattr(taskhome, 'SCF_MAX_PAGES', 3)
    monkeypatch.setattr(taskhome, 'SCF_PER_PAGE', 2)
...
def test_missing_pagination_metadata_falls_back_to_short_page(scf, clean_state, monkeypatch):
    ...
    monkeypatch.setattr(taskhome.requests, 'get', get)
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    assert len(scf.requests) == 1
...
def test_watermark_is_taken_before_the_fetch(scf, clean_state):
    ...
    taskhome.poll_scf_listener(now)
    assert taskhome.listeners['scf']['last_check'] == '2026-03-05T12:00:00Z'
...
def test_fetch_failure_does_not_advance_the_watermark(scf, clean_state):
    ...
    taskhome.listeners['scf']['last_check'] = '2026-03-05T11:00:00Z'
    scf.error = RuntimeError('connection reset')
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    assert taskhome.listeners['scf']['last_check'] == '2026-03-05T11:00:00Z'


def test_failures_back_off_and_recover(scf, clean_state):
    scf.error = RuntimeError('boom')
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    state = taskhome.listeners['scf']
    assert state['consecutive_failures'] == 1
    assert 'backoff_until' in state

    # Inside the backoff window: no request attempted.
    before = len(scf.requests)
    taskhome.poll_scf_listener(utc('2026-03-05T12:01:00'))
    assert len(scf.requests) == before

    # After it expires and the API recovers.
    scf.error = None
    scf.set_issues([issue(1)])
    taskhome.poll_scf_listener(utc('2026-03-05T13:00:00'))
    assert taskhome.listeners['scf']['consecutive_failures'] == 0
    assert 'backoff_until' not in taskhome.listeners['scf']


def test_backoff_grows_with_repeated_failures(scf, clean_state):
    scf.error = RuntimeError('boom')
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    first = taskhome.parse_utc(taskhome.listeners['scf']['backoff_until'])

    taskhome.listeners['scf'].pop('backoff_until')
    taskhome.poll_scf_listener(utc('2026-03-05T12:00:00'))
    second = taskhome.parse_utc(taskhome.listeners['scf']['backoff_until'])
...
def test_unprinted_issue_is_retried_next_cycle(scf, clean_state):
    ...
    scf.set_issues([issue(1)])
    assert taskhome.poll_scf_listener(utc('2026-03-05T12:00:00')) == 0
    assert taskhome.listeners['scf'].get('seen') in (None, [])
