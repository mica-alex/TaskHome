...
def test_slices_the_requested_page():
    page = taskhome.web.pagination.paginate(list(range(100)), page=2, per_page=25)
...
def test_last_page_may_be_short():
    page = taskhome.web.pagination.paginate(list(range(72)), page=3, per_page=25)
...
def test_empty_history():
    page = taskhome.web.pagination.paginate([], page=1, per_page=25)
    assert page['items'] == []
    assert page['pages'] == 1
    assert page['total'] == 0
    assert page['start'] == 0
...
def test_out_of_range_page_clamps_rather_than_emptying(requested):
    ...
    page = taskhome.web.pagination.paginate(list(range(50)), page=requested, per_page=25)
...
def test_unsupported_page_size_falls_back(size):
    page = taskhome.web.pagination.paginate(list(range(50)), page=1, per_page=size)
    assert page['per_page'] == taskhome.web.pagination.DEFAULT_PAGE_SIZE


def test_supported_page_sizes_are_honoured():
    for size in taskhome.web.pagination.PAGE_SIZES:
        assert taskhome.web.pagination.paginate(list(range(500)), 1, size)['per_page'] == size


# --- page_numbers -------------------------------------------------------------

def test_short_runs_show_every_page():
    assert taskhome.web.pagination.page_numbers(1, 5) == [1, 2, 3, 4, 5]


def test_long_runs_elide_the_middle():
    numbers = taskhome.web.pagination.page_numbers(10, 20)
...
def test_first_and_last_are_always_reachable():
    for page in (1, 7, 20):
        numbers = taskhome.web.pagination.page_numbers(page, 20)
        assert 1 in numbers and 20 in numbers


def test_no_duplicate_page_numbers():
    numbers = [n for n in taskhome.web.pagination.page_numbers(2, 20) if n is not None]
...
def test_filter_by_type():
    ...
    assert len(taskhome.web.pagination.filter_history(records, kind='task')) == 2
    assert len(taskhome.web.pagination.filter_history(records, kind='scf')) == 1


def test_unknown_type_is_ignored_rather_than_matching_nothing():
    records = [task_record(1), scf_record(2)]
    assert len(taskhome.web.pagination.filter_history(records, kind='nonsense')) == 2


def test_search_matches_task_titles():
    records = [task_record(1), task_record(2)]
    assert len(taskhome.web.pagination.filter_history(records, query='Task 1')) == 1


def test_search_matches_scf_address_and_description():
    records = [scf_record(7), task_record(1)]
    assert len(taskhome.web.pagination.filter_history(records, query='Elm')) == 1
    assert len(taskhome.web.pagination.filter_history(records, query='hole in the road')) == 1


def test_search_is_case_insensitive():
    assert len(taskhome.web.pagination.filter_history([scf_record(1)], query='POTHOLE')) == 1


def test_all_terms_must_match():
    records = [scf_record(1), task_record(1)]
    assert len(taskhome.web.pagination.filter_history(records, query='pothole elm')) == 1
    assert len(taskhome.web.pagination.filter_history(records, query='pothole nonsense')) == 0


def test_search_matches_id():
    assert len(taskhome.web.pagination.filter_history([scf_record(5)], query='1005')) == 1


def test_search_and_type_combine():
    records = [scf_record(1), task_record(1)]
    assert len(taskhome.web.pagination.filter_history(records, query='1', kind='task')) == 1


def test_filter_preserves_order():
    records = [task_record(n) for n in range(10)]
    assert taskhome.web.pagination.filter_history(records, query='Task') == records


def test_records_missing_fields_do_not_crash():
    assert taskhome.web.pagination.filter_history([{'type': 'task'}], query='anything') == []


# --- the route ----------------------------------------------------------------

def test_page_renders_only_one_page_of_records(client, clean_state):
    taskhome.state.history.extend(task_record(n) for n in range(200))
...
def test_search_narrows_the_table(client, clean_state):
    taskhome.state.history.extend([task_record(1), scf_record(2)])
...
def test_no_matches_shows_an_empty_state(client, clean_state):
    taskhome.state.history.extend([task_record(1)])
...
def test_pager_links_carry_the_search(client, clean_state):
    ...
    taskhome.state.history.extend(task_record(n) for n in range(100))
...
def test_malformed_query_strings_do_not_500(client, clean_state, qs):
    taskhome.state.history.extend(task_record(n) for n in range(30))
    assert client.get(f'/task_page?{qs}').status_code == 200


def test_history_is_not_mutated_by_viewing(client, clean_state):
    taskhome.state.history.extend(task_record(n) for n in range(50))
    before = list(taskhome.state.history)
    client.get('/task_page?q=Task&page=2')
    assert taskhome.state.history == before
