...
def test_simple_recurrences(recurring, start, expected):
    assert taskhome.calculate_next(start, recurring) == expected


def test_custom_selects_next_matching_weekday():
    # 2026-03-10 is a Tuesday; ask for Mondays (0) and Wednesdays (2).
    assert taskhome.calculate_next('2026-03-10T09:00:00', 'custom', [0, 2]) == \
        '2026-03-11T09:00:00'
...
def test_none_returns_input_unchanged():
    ...
    assert taskhome.calculate_next(start, 'none') == start


def test_unknown_recurrence_returns_input_unchanged():
    start = '2026-03-10T09:00:00'
    assert taskhome.calculate_next(start, 'fortnightly-ish') == start
...
def test_custom_with_no_valid_days_terminates(days):
    ...
    start = '2026-03-10T09:00:00'
    assert taskhome.calculate_next(start, 'custom', days) == start


def test_custom_ignores_invalid_days_but_uses_valid_ones():
    result = taskhome.calculate_next('2026-03-10T09:00:00', 'custom', [99, 2])
...
def test_future_task_is_untouched(make_task):
    ...
    next_time, missed = taskhome.advance_schedule(task, dt('2026-03-09T00:00:00'))
...
def test_rolls_forward_past_now_and_reports_missed(make_task):
    ...
    next_time, missed = taskhome.advance_schedule(task, dt('2026-03-05T12:00:00'))
...
def test_boundary_exactly_due_counts_as_missed(make_task):
    ...
    next_time, missed = taskhome.advance_schedule(task, dt('2026-03-05T09:00:00'))
...
def test_overdue_oneoff_returns_no_future_occurrence(make_task):
    ...
    next_time, missed = taskhome.advance_schedule(task, dt('2026-03-05T00:00:00'))
...
def test_overdue_custom_with_empty_days_raises_not_hangs(make_task):
    ...
    with pytest.raises(taskhome.ScheduleError, match='did not advance'):
        taskhome.advance_schedule(task, dt('2026-03-05T00:00:00'))
...
def test_iteration_ceiling_is_enforced(make_task, monkeypatch):
    ...
    with pytest.raises(taskhome.ScheduleError, match='exceeded'):
        taskhome.advance_schedule(task, dt('2026-03-05T00:00:00'), max_iterations=10)
...
def test_backwards_recurrence_raises(make_task, monkeypatch):
    ...
    monkeypatch.setattr(taskhome, 'calculate_next',
                        lambda s, r, d=None: '2019-01-01T09:00:00')
    task = make_task('2020-01-01T09:00:00', 'daily')
    with pytest.raises(taskhome.ScheduleError, match='backwards'):
        taskhome.advance_schedule(task, dt('2026-03-05T00:00:00'))
...
def test_parse_naive_time_roundtrips():
    assert taskhome.parse_task_time('2026-03-10T09:00:00') == dt('2026-03-10T09:00:00')


def test_parse_strips_timezone_to_local():
    """Aware values are normalised so comparisons stay in one frame (P0-3)."""
    parsed = taskhome.parse_task_time('2026-03-10T09:00:00+00:00')
...
def test_parse_rejects_garbage(value):
    with pytest.raises(ValueError):
        taskhome.parse_task_time(value)
...
def test_double_seconds_suffix_is_tolerated_on_modern_python():
    ...
    """
    assert taskhome.parse_task_time('2026-03-10T21:00:00:00') == dt('2026-03-10T21:00:00')
